 ...
 * Description  ViewModel for the list of tasks
...
public class TasksViewModel {
    ...
    public Observable<TasksUiModel> getUIModel() {
        return getTaskItems().doOnSubscribe(() -> mLoadingIndicatorSubject.onNext(true))
     ...
     * @return
     */
    public TasksUiModel constructTasksModel(Pair<List<TaskItem>, TasksFilterType> pair) {
    ...
    private Observable<List<TaskItem>> getTaskItems() {
        return Observable.combineLatest(mTasksRepository.getTasks(),
                mFilter, Pair::create).flatMap(
                pair -> Observable.from(pair.first)
    ...
    private TaskItem constructTaskItem(Task task) {
        ...
        return new TaskItem(task, background, () -> handleTaskTaped(task),
    ...
    private Completable completeTask(Task task) {
        return mTasksRepository.completeTask(task).doOnCompleted(() -> mSnackbarText.onNext(R.string.task_marked_complete));
    }

    private Completable activateTask(Task activeTask) {
        return mTasksRepository.activateTask(activeTask).doOnCompleted(
    ...
    public Completable forceUpdateTasks() {
        ...
        return mTasksRepository.refreshTasks().doOnTerminate(() -> mLoadingIndicatorSubject.onNext(false));
    }
     ...
     */
    public void handActivityResult(int requestCode, int resultCode) {
