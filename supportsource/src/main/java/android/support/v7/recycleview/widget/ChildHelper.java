...
public class ChildHelper {
    ...
    void removeViewAt(int index) {
        ...
        final View view = mCallback.getChildAt(index);
        if (view == null) {
            return;
        }
    ...
    View findHiddenNonRemovedView(int position, int type) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (holder.getLayoutPosition() == position && !holder.isInvalid() && !holder.isRemoved() && (type == RecyclerView.INVALID_TYPE || holder.getItemViewType() == type)) {
                return view;
            }
        }
        return null;
    ...
    void attachViewToParent(View child, int index, ViewGroup.LayoutParams layoutParams, boolean hidden) {
        ...
        mCallback.attachViewToParent(child, index, layoutParams);
    }
