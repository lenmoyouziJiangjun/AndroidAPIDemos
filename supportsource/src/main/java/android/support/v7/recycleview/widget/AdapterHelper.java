...
public class AdapterHelper implements OpReorderer.Callback {
    ...
    private void applyRemove(UpdateOp op) {
        ...
        for (int position = op.positionStart; position < tmpEnd; position++) {
            ...
            RecyclerView.ViewHolder viewHolder = mCallback.findViewHolder(position);
            if (viewHolder != null || canFindInPreLayout(position)) {
    ...
    public UpdateOp obtainUpdateOp(int cmd, int startPosition, int itemCount, Object payload) {
        ...
        if (op == null) {
            op = new UpdateOp(cmd, startPosition, itemCount, payload);
        } else {
            op.cmd = cmd;
            op.positionStart = startPosition;
