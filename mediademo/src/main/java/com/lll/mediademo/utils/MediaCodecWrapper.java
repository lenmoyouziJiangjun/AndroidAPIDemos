...
public class MediaCodecWrapper {
    ...
    public void setOutputFormatChangedListener(final OutputFormatChangedListener changedListener,
                                               Handler handler) {
        mOutputFormatChangedListener = changedListener;
        Looper looper;
        mHandler = handler;
        if (changedListener != null && mHandler == null) {
    ...
    public boolean writeSample(final MediaExtractor extract,
        ...
        if (!mAvailableInputBuffers.isEmpty()) {
            ...
            int size = extract.readSampleData(buffer, 0);
            if (size <= 0) {
            ...
            if (!isSecure) {
                ...
                extract.getSampleCryptoInfo(cryptoInfo);
